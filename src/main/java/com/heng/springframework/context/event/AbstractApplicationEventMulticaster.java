...
public abstract class AbstractApplicationEventMulticaster implements ApplicationEventMulticaster, BeanFactoryAware {
    ...
    /**
     * 注册监听器，当 multicastEvent方法被调用时，会根据已经注册的监听器对事件进行广播。
     ...
     * 监听器是否对该事件感兴趣
     * event表示事件
     * @param applicationListener applicationListener表示事件的监听器，其所携带的泛型表示被监听时间的泛型。如果event是该泛型类型的子类，则表示该事件为被监听事件
     * @param event event 表示时间
     */
